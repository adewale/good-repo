...
```sh
python3 skill-eval-harness/skill_benchmark.py validate good-repo/evals/shared-benchmark.json
```

Prepare paired run tasks:

```sh
python3 skill-eval-harness/skill_benchmark.py prepare good-repo/evals/shared-benchmark.json --split tune --out /tmp/good-repo-tasks.jsonl
```

Include ablation variants when running a focused regression check:

```sh
python3 skill-eval-harness/skill_benchmark.py prepare good-repo/evals/shared-benchmark.json --split tune --include-ablations --out /tmp/good-repo-ablation-tasks.jsonl
```
...
```sh
python3 skill-eval-harness/skill_benchmark.py benchmark good-repo/evals/shared-benchmark.json --runs good-repo/eval-runs/latest --out /tmp/good-repo-benchmark.json
```
